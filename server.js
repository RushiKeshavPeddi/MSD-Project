...
const indianMovieTitles = [
  'Pushpa', 'RRR', 'KGF', 'Baahubali', 'Jawan', 'Pathaan', 'Dangal',
  'PK', 'Bajrangi Bhaijaan', 'Sultan', 'Tiger Zinda Hai', 'War',
  'Sanju', 'Padmaavat', '3 Idiots', 'Dhoom', 'Chennai Express',
  'Happy New Year', 'Kick', 'Bang Bang', 'Ek Tha Tiger', 'Yeh Jawaani Hai Deewani'
];
...
const getCityRegion = (cityId) => {
  const regionMap = {
    ...
    'North': [2, 10, 11, 12, 21, 22, 23, 25, 26, 30, 34, 36, 43, 46, 47, 53, 54, 56, 57, 63, 64, 65, 67, 75, 81, 85, 86, 88, 90, 99],
  ...
  for (const [region, cityIds] of Object.entries(regionMap)) {
    if (cityIds.includes(parseInt(cityId))) {
      return region;
    }
  }
  return 'North';
  ...
  try {
    ...
    if (response.data.Response === 'True') {
      return {
        ...
        poster: response.data.Poster !== 'N/A' ? response.data.Poster : 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=400&h=600&fit=crop',
...
const getRecentMoviesFromOMDB = async (cityId) => {
  ...
  const moviesToFetch = indianMovieTitles
...
const theaters = {
  1: [
    { id: 1001, name: 'PVR Phoenix Mumbai', showtimes: ['10:00 AM', '1:30 PM', '5:00 PM', '8:30 PM'], ticketPrice: 350 },
    { id: 1002, name: 'Inox Nariman Point', showtimes: ['11:00 AM', '2:30 PM', '6:00 PM', '9:30 PM'], ticketPrice: 380 },
  ...
  3: [
    ...
    { id: 3003, name: 'Cinepolis Royal Meenakshi', showtimes: ['10:30 AM', '2:00 PM', '5:30 PM', '9:00 PM'], ticketPrice: 250 }
  ],
  4: [
    { id: 4001, name: 'AMB Cinemas', showtimes: ['10:00 AM', '1:30 PM', '5:00 PM', '8:30 PM'], ticketPrice: 280 },
    { id: 4002, name: 'PVR Forum Sujana Mall', showtimes: ['11:00 AM', '2:30 PM', '6:00 PM', '9:30 PM'], ticketPrice: 300 },
...
app.get('/api/theaters/:cityId/:movieId', (req, res) => {
  ...
  let cityTheaters = theaters[cityId] || [
    { id: parseInt(cityId) * 100 + 1, name: `PVR Cinemas`, showtimes: ['10:00 AM', '1:30 PM', '5:00 PM', '8:30 PM'], ticketPrice: 200 },
    { id: parseInt(cityId) * 100 + 2, name: `Inox Multiplex`, showtimes: ['11:00 AM', '2:30 PM', '6:00 PM', '9:30 PM'], ticketPrice: 220 },
    { id: parseInt(cityId) * 100 + 3, name: `Cinepolis`, showtimes: ['10:30 AM', '2:00 PM', '5:30 PM', '9:00 PM'], ticketPrice: 180 }
  ];
...
app.get('/api/booked-seats/:theaterId/:showtime', (req, res) => {
  ...
  const theaterBookings = bookings.filter(
    b => b.theaterId == theaterId && b.showtime === showtime
  );
